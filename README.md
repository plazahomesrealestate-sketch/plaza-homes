[index.html](https://github.com/user-attachments/files/27355360/index.html)
<!DOCTYPE html>
                            ...
                            <div class="glass rounded-2xl p-6 sticky top-24">
                                ...
                                <form id="add-property-form" onsubmit="event.preventDefault(); handleAddProperty();">
                                    <div class="space-y-4 max-h-[65vh] overflow-y-auto hide-scrollbar pr-1">
                                        ...
                                        <div>
                                            <label class="block text-xs mb-1 text-gray-400">رفع صورة</label>
                                            <input type="file" id="prop-image-file" accept="image/*" class="w-full px-3 py-2 rounded-lg text-sm text-gray-400 file:mr-4 file:py-1 file:px-3 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-gold file:text-dark hover:file:bg-gold-light" onchange="handleImagePreview(this)">
                                            <p class="text-[10px] text-gray-500 mt-1">أو أدخل رابط الصورة أدناه</p>
                                        </div>
                                        <div>
                                            <label class="block text-xs mb-1 text-gray-400">رابط الصورة</label>
                                            <input type="url" id="prop-image-url" class="w-full px-3 py-2 rounded-lg text-sm" placeholder="https://..." oninput="updatePreview()">
                                        </div>
                                        <div>
    ...
    <script>
        ...
        async function handleAddProperty() {
            ...
            try {
                ...
                const imageUrlInput = document.getElementById('prop-image-url').value;
                let imageUrl = imageUrlInput;

                // Upload image if file selected
                if (fileInput.files && fileInput.files[0]) {
                    const file = fileInput.files[0];
                    const storageRef = window.firebaseStorageRef(window.storage, 'properties/' + Date.now() + '_' + file.name);
